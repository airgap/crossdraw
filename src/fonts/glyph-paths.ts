/**
 * Path-based variable-font rendering for Canvas 2D.
 *
 * ### Why this exists
 *
 ...
 * every failure mode we could think of as an executable record.)
 *
 * ### The escape hatch
 *
 * fontkit can apply variation deltas to glyph outlines directly when it
 * owns the raw TTF bytes. Given a TTF, `font.getVariation({wdth: 100})`
 * returns a view whose `glyph.path.commands` reflect the variation —
 * those commands are just moveTo / lineTo / curveTo pairs we can replay
 * into a `ctx.fill()`. Bypassing the browser text stack entirely lets us
 * drive *any* axis.
 *
 * ### The WOFF2 problem
 *
 ...
 * (working) TTF path. (`scripts/build-fontkit-vendor.ts` bundles and
 * patches wawoff2 so its Module object escapes its CJS closure in the
 * browser — see that script's comments for the grisly details.)
 *
 * ### Caching strategy
 *
 * Loading is async and expensive; rendering must be synchronous. The
 * module therefore exposes a `getPathText()` handle that reports
 * `ready === false` while loading is in-flight, fires a render callback
 * when work completes, and serves synchronous glyph paths once cached:
 *
 ...
 *      (family, sourceUrl, variationKey).
 *   3. **Glyph paths** — `glyph.path.commands` → `Path2D`, cached by
 *      (family, sourceUrl, variationKey, codePoint).
 *
 * The expensive work happens once per (font, axis values) tuple; sliders
 * that sweep through values produce one decode per discrete axis setting
 * visited, after which rasterization is just `ctx.fill(cachedPath2D)`.
 *
 * When the viewport calls `getPathText()` before the font is ready, we
 * kick off the load chain and call the registered render callback when
 * everything resolves — see `setStyledFontRenderCallback` in loader.ts.
 */
...
// fontkit and wawoff2 types — we don't depend on fontkit's published .d.ts
// because the bundled browser module ships without them. Minimal shapes
// only.
...
interface GlyphEntry {
  path: Path2D
  advanceWidth: number // in em units
}
/** (family, sourceUrl, variationKey, codePoint) → rendered glyph. */
 ...
 * still renders correctly (just without non-wght axes).
 *
...
function variationKey(axes: FontVariationAxis[]): string {
  // Sort so order-independent and deterministic.
  return axes
    .slice()
...
function getGlyphEntry(
  ...
  if (cached) return cached
  const glyph = variant.glyphForCodePoint(codePoint)
  const entry: GlyphEntry = {
    path: commandsToPath2D(glyph.path.commands),
...
export interface PathText {
  ...
  /** Measure a string's advance width in px. */
...
/**
 * Determine whether a variation set needs path-based rendering.
 *
 ...
 * the fast native path. Any other axis at a non-default value forces us
 * onto the path renderer. (We still include `wght` in the variation
 * coordinates we feed fontkit so the two paths agree.)
 */
export function needsPathRendering(variations?: FontVariationAxis[]): boolean {
 ...
 * axis values. Returns synchronously; the handle reports `ready=false`
 * while fonts/bundles are loading in the background, then the registered
 * render callback fires.
 */
export function getPathText(family: string, variations: FontVariationAxis[] | undefined, sizePx: number): PathText {
  ...
  const vkey = variations && variations.length > 0 ? variationKey(variations) : ''
  const unitsPerEm = font.unitsPerEm
  const emToPx = sizePx / unitsPerEm

  function glyphFor(ch: string): GlyphEntry {
    return getGlyphEntry(family, sourceUrl!, vkey, ch.codePointAt(0)!, variant)
  }

  return {
    ready: true,
    measureChar(ch) {
      return glyphFor(ch).advanceWidth * emToPx
    },
    measureWidth(str) {
      let w = 0
      for (const ch of str) w += glyphFor(ch).advanceWidth
      return w * emToPx
    },
    fillText(ctx, str, x, y, letterSpacing) {
      ...
      for (const ch of str) {
        const g = glyphFor(ch)
        ctx.save()
        ctx.translate(cursorEm, 0)
        ctx.fill(g.path)
        ctx.restore()
        cursorEm += g.advanceWidth + lsEm
      }
      ctx.restore()
    },
  }
