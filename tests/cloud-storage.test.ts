...
describe('Cloud Storage - File Index Operations', () => {
  describe('addFileEntry', () => {
    ...
    it('should append to existing entries', () => {
      const existing = createTestEntry({ id: 'existing1', name: 'First.design' })
      const index: FileIndex = { files: [existing] }
      const newEntry = createTestEntry({ id: 'new2', name: 'Second.design' })
  ...
  describe('updateFileEntry', () => {
    it('should update fields of a matching entry', () => {
      const entry = createTestEntry({ id: 'up1', name: 'Old.design', size: 100 })
      ...
      expect(result.files).toHaveLength(1)
      expect(result.files[0]!.name).toBe('New.design')
    ...
    it('should not affect other entries', () => {
      const entry1 = createTestEntry({ id: 'a1', name: 'A.design' })
      const entry2 = createTestEntry({ id: 'b2', name: 'B.design' })
      const index: FileIndex = { files: [entry1, entry2] }
      const result = updateFileEntry(index, 'a1', { name: 'Updated.design' })
      expect(result.files[0]!.name).toBe('Updated.design')
      expect(result.files[1]!.name).toBe('B.design')
    })
    ...
    it('should return unchanged index if id not found', () => {
      ...
      expect(result.files).toHaveLength(1)
      expect(result.files[0]!.name).toBe('TestDoc.design')
    })

    it('should not mutate the original index', () => {
      const entry = createTestEntry({ id: 'mut1', name: 'Before.design' })
      const index: FileIndex = { files: [entry] }
      const result = updateFileEntry(index, 'mut1', { name: 'After.design' })
      expect(index.files[0]!.name).toBe('Before.design')
      expect(result.files[0]!.name).toBe('After.design')
    })
  })
  ...
  describe('removeFileEntry', () => {
    ...
    it('should keep other entries', () => {
      const entry1 = createTestEntry({ id: 'k1', name: 'Keep.design' })
      const entry2 = createTestEntry({ id: 'r2', name: 'Remove.design' })
      const entry3 = createTestEntry({ id: 'k3', name: 'AlsoKeep.design' })
